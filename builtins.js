...
class RammerApp {
	constructor(txt) {
		...
		this.lay.SetMargins(0, layotherstatusbar.GetHeight(), 0, 0)
		this.lay.SetSize(1, PX2DSW(app.GetScreenHeight() - 195 - 160 - 90 - 300))
		this.state = "unused"
		this.ifadded = false
...
class RammerRepository {
	...
	DownloadPackage(pkg) {
		...
		this.tmp.SetOnError(function() {
			return false
		})
		this.tmp.SetOnComplete(function() {
			return true
		})
	}
...
class RammerAdditionalTextUtilities {
	...
	blink(timer) {
		...
		this.placer = []
		for (this.a = 0; this.a < this.prev.length; this.a++) {
			this.placer.push(" ")
		}
...
class RammerVideo {
	constructor(lay, w, h) {
		...
		this.vault.SetOnTouchDown(function() {
			this.ulay.AddChild(this.ctrllay)
		})
		this.ctrllay.SetOnTouchDown(function() {
			this.ulay.RemoveChild(this)
		})
		// bottom ctrls
...
function RammerCPUUsage() {
	return app.ReadFile("/proc/loadavg").split(" ")
}
...
class RammerFileSelect {
	...
	back() {
		if ("/" + this.dir.join("/") != "/" + dirf.join("/") && this.shsd == false) {
			this.dir.pop()
		}
...
class RammerAudioPlayer {
	constructor(lay, w) {
		...
		this.playbtn.SetOnTouch(() => {
			this.play()
		})
		this.pausebtn.SetOnTouch(() => {
			this.pause()
		})
		this.stopbtn.SetOnTouch(() => {
			this.stop()
		})
		this.volskb.SetOnChange((c) => {
			this.setvolume(c)
		})
	}
	setfile(file) {
...
class RammerImageCropper {
	constructor(image) {
		...
		this.cb.oper = this
		this.cb.SetOnTouch(function() {
			this.oper.oncomplete(this.oper.image);
			this.oper.lay.close()
		})
...
class RammerNotification {
	constructor(title, text, func, icon, sound, options) {
		...
		this.plimg = app.CreateImage(this.icon != null ? this.icon : "Img/rammer.png", 0.11)
		...
		this.nlay.SetOnTouchUp(function() {
			app.RemoveLayout(this)
			clearTimeout(this.nid)
			if (typeof this.func != "undefined" && this.func != null) {
				this.func()
			}
		})
...
function RammerSystem_GetRemoteVersion() {
	...
	try {
		...
		let tmp = new RammerNotification("Updates", "Failed to check updates...", () => {}, null, null, null).trigger()
		return null
	}
}
...
function RammerDownload_Confirm(url, path, name, notif) {
	'use strict'
	...
	ldr_lay_btns_dwn.SetOnTouch(function() {
		...
		if (!notif) {
			...
			jdwn.Download(url, ldr_lay_path_path.GetText(), ldr_lay_name_name.GetText())
		}
	})
...
function RammerVFSCreateObject(path) {
	let pt = path.split("/")
	let file = pt.pop()
	pt = pt.join('')
	let gt = RammerVFSGet(pt)
	...
	gt['contents'][file].read = function() {
		alert("Hello!")
	}
}

function PX2DSW(pixels) {
	return 1 / app.GetScreenWidth() * pixels
}

function PX2DSH(pixels) {
	return 1 / app.GetScreenHeight() * pixels
}
